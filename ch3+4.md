...
The mean threshold across folds was **0.448** (SD = 0.062, range: 0.37-0.50), reflecting an intentional downward adjustment from the default 0.50 to prioritize sensitivity in a screening setting. This aligns with clinical guidelines that favor high sensitivity for initial T2DM screening, with confirmatory testing (FPG, OGTT) reserved for screen-positive cases.

**Clarification on Sensitivity Target vs. Achieved Sensitivity:** The screening-optimized strategy enforces a per-fold constraint of Sensitivity ≥ 0.80 and Specificity ≥ 0.40. However, when no threshold satisfies both constraints simultaneously (which occurred in some LOGO folds due to data distribution), the screening strategy fails for that fold. The composite scoring mechanism then selects the best among Youden's J, G-Mean, or the screening strategy (if valid). The reported mean sensitivity of 0.748 reflects the aggregate performance across all folds and strategies selected—not a failure to meet the screening constraint, but rather the result of a multi-strategy optimization process where the screening strategy was not always the composite winner.
...
**Expert-Specified Feature Weights (Literature-Backed Justification):** The clustering weights function as feature scaling multipliers before Euclidean distance computation—a standard domain-knowledge injection technique. Higher weights amplify a feature's influence on cluster separation. The following weights are applied to the standardized features, reflecting their relative clinical magnitude for T2DM subtype separation:
...
**Table 3.3 — Expert Feature Weights and Rationale Summary**
...
The logistic regression model demonstrated clinically acceptable discriminative performance under nested LOGO validation, achieving an AUC-ROC of **0.7267** (95% CI: 0.700–0.753) and a sensitivity of **0.748** (95% CI: 0.717–0.777) at the optimized screening threshold of 0.448. Specificity was **0.551**, reflecting the sensitivity-biased threshold design appropriate for population screening where false negatives (missed at-risk patients) carry higher clinical cost than false positives (over-referral for confirmatory testing). The F1 score of **0.699** indicates balanced precision-recall trade-offs at the selected threshold.
...
1. **Marginally superior mean fold AUC** across LOGO folds (0.7267 vs. 0.7142 for RF, 0.7026 for LightGBM)
...
- **Confidence Indicator**: Visual gauge based on probability threshold (0.448)
