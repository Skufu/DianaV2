...
- **BMI: 1.5 (Moderate-High Weight — MOD Cluster Anchor):** BMI is weighted slightly lower than TG and WC because its contribution partially overlaps with Waist Circumference (r > 0.80). However, it independently defines the MOD (Mild Obesity-Related Diabetes) cluster, making it an indispensable signal for separation.
...
| **BMI** | 1.5 | #3 | Defines MOD cluster; ΔFasting glucose 0.112/SD; overlaps WC | Important but partially redundant with WC; moderate amplification |
...
| **Age** | 1.0 | #5 | MARD defined by age; compressed variance in menopausal cohort | Baseline — cohort is already age-restricted; metabolic features dominate |
...
- **Observation**: Team members initially expected MOD to represent "moderate obesity" rather than the data-driven cluster centroid with BMI=42.19 (severe obesity)
