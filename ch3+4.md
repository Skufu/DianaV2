...
1. **User Input** (Frontend Layer):
   - Clinician enters biomarker values in React assessment form (`frontend/src/components/user/AssessmentForm.jsx`)
...
2. **Authentication & Authorization** (Backend Layer):
   ...
   - RBAC middleware checks clinician role (`middleware/rbac.go`)
...
| **Usability** | Clinician-facing UI evaluation | [TBD: SUS or QUIS survey with N clinicians] | 🔄 Not complete |
...
The functional suitability assessment confirms that DIANA provides **all required functionality** for diabetes risk screening among menopausal women, with optional clinician-mode features for 3-class classification.
...
1. **User-Centered Design**: UI components designed for clinician workflows (not developers)
...
The final usability evaluation may indicate **structured user testing** with N clinicians, measuring:

- **Task Completion Rate**: Percentage of clinicians who successfully complete risk assessment workflow
...
4. **User Accountability**: Unique user IDs associate all actions with authenticated clinicians
...
The DIANA application implements a streamlined clinician-facing workflow that transforms raw biomarker data into actionable clinical insights through a four-stage user journey. This section documents the implemented UI workflow, component integration, and observed user interaction patterns.

**Four-Stage Assessment Workflow**

**Stage 1 - Dashboard Overview (Entry Point)**

The main clinician dashboard (`Dashboard_user.jsx`) serves as the central hub displaying patient population summaries and individual risk indicators. Upon authentication, clinicians see:

- **Risk Distribution Summary**: Color-coded overview showing proportion of patients across Normal (green), Moderate (yellow/amber), and High (red) risk categories
- **Recent Assessments**: Chronological list of the most recent patient entries with associated timestamps and risk scores
...
*Description: Clinician dashboard showing risk distribution cards and recent assessment list. Risk indicators use green/yellow/red color coding per Gestalt similarity principle.*
...
The PersonalTrends component (`PersonalTrends.jsx`) enables longitudinal tracking of patient biomarker evolution and risk trajectory:
...
- **Objective**: Enter biomarker data for a hypothetical patient and submit for risk prediction
...
1. **Risk Prediction Accuracy**: Based on comparison of DIANA predictions against your clinical judgment for hypothetical patient cases
2. **SHAP Explanation Clarity**: Based on how understandable and actionable the feature contributions are for patient counseling
3. **Clinical Workflow Integration**: Based on how seamlessly DIANA fits into your existing patient assessment and documentation workflow
...
|-----------|-------------|------------------------|----------------|
| **Proximity** | Elements placed together are perceived as related | Patient biomarker cards grouped visually in dashboard; related form fields clustered in assessment forms | **[PLACEHOLDER: Screenshot]** |
...
| **Figure-Ground** | Differentiates figure from background | High-risk patients highlighted with bold red cards against neutral dashboard background; active navigation items distinguished with accent color | **[PLACEHOLDER: Screenshot]** |
| **Focal Point** | Unique elements draw attention | Risk score displayed as large, bold number on patient detail page; SHAP waterfall plot uses contrasting colors to highlight top contributors | **[PLACEHOLDER: Screenshot]** |
...
| **Common Region** | Elements within same boundary perceived as group | Card-based layout groups related information (patient info, biomarkers, recommendations) within distinct borders | **[PLACEHOLDER: Screenshot]** |
