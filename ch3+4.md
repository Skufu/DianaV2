...
**Interpretation:** The resulting AUC-ROC of 0.72 should therefore be interpreted as a **conservative temporal generalization estimate**, not a standard k-fold cross-validation figure. Studies using random k-fold splits on temporal health data consistently report optimistically biased AUC values compared to temporal-based estimates due to temporal correlation and data leakage (Futoma et al., 2020); for equivalent feature sets on NHANES, standard k-fold approaches can overestimate performance by 0.05-0.15.
...
- **LDL: 2.5 (Highest Weight — Strongest Lipid Differentiator):** LDL receives the highest weight because it is the most clinically potent lipid biomarker for cardiometabolic risk stratification across diabetes subtypes. It independently predicts T2DM risk (e.g., OR = 1.12 per SD increase, and HR = 0.90 per 10 mg/dL lower trajectory) and serves as the principal differentiator in K-Means-based metabolic clustering of high-risk obese individuals. Furthermore, it bidirectionally highlights atherogenic dyslipidemia while offering a diabetes risk signal.
- **Triglycerides: 2.0 (High Weight — Insulin Resistance Core):** Triglycerides (TG) share the second-highest weight with Waist Circumference because they are jointly the dominant surrogates for insulin resistance. Evidence indicates that up to 75% of insulin resistance can be attributed to TG levels, and TG/WC pairs consistently dominate metabolic syndrome factor analysis (often explaining >60% of metabolic variance together).
...
|---|---|---|---|---|
| **LDL** | 2.5 | #1 | OR = 1.12 per SD; HR = 0.90 per 10mg/dL; best discriminator in K-Means | Most bidirectionally discriminative lipid; best cluster separator |
| **Triglycerides** | 2.0 | #2 (tied)| 75% of IR attributed to TG; stronger in women; 61.3% variance | Primary IR surrogate; TG/WC pair dominates MetS factor structure |
| **Waist Circumference**| 2.0 | #2 (tied)| B = 0.024 for HOMA-IR; loads on same factor as HOMA-IR | Central adiposity independent signal; co-dominant with TG for IR |
...
| **HDL** | 1.2 | #4 | OR = 0.69/mmol/L (MR-confirmed); 16% T2DM risk reduction per SD | Inverse/protective signal; lower variance; amplifies TG's direction |
...
- **BMI & Waist Circumference:** BMI consistently emerges as the primary predictor in logistic regression screening models (e.g., OR = 1.14 per 1-unit increase). Waist circumference provides an independent central adiposity marker, where each 1 cm increase raises relative risk by 8%, even for individuals with normal BMI.
...
Studies using random k-fold splits or single train-test splits on temporal health data consistently report AUC values 0.05-0.15 higher than LOGO-based estimates for equivalent feature sets, due to temporal correlation within cycles and data leakage (Futoma et al., 2020). Under these conditions, AUC >= 0.70 constitutes clinically acceptable screening performance for a non-circular surrogate marker model (Maxim et al., 2014).
...
Tenenbaum, A., Tanne, D., Fisman, E. Z., Adler, Y., & Motro, M. (2006). Atherogenic dyslipidemia in metabolic syndrome and type 2 diabetes. *Cardiovascular Diabetology*, 5(1), 1-8.
...
Ahmed, F., et al. (2021). Impact of triglycerides and waist circumference on insulin resistance and β-cell function in non-diabetic first-degree relatives of type 2 diabetes. PMC8207623.

Ahlqvist, E., et al. (2018). Novel subgroups of adult-onset diabetes and their association with outcomes: a data-driven cluster analysis of six variables. *Lancet Diabetes Endocrinol*. PMID: 29503172.

Guo, W., et al. (2020). Lipid accumulation product is a strong predictor of insulin resistance in US adults. *J Clin Endocrinol Metab*.

Huang, J., et al. (2023). Association between serum LDL‐C concentrations and risk of diabetes: A prospective cohort study. PMC10590678.

Li, L., et al. (2019). K-means clustering of overweight and obese population using quantile-transformed metabolic data. PMC6711566.
