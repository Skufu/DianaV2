...
**Stage 2 - K-Means Subtyping (Stratifier):** K-Means clustering (K=4) is applied **exclusively to at-risk patients** (those classified as Pre-diabetic or Diabetic, y = 1), not the full cohort. This is methodologically correct because clustering aims to stratify the metabolic heterogeneity within the at-risk population, not to separate at-risk from normal subjects (which is the binary classifier's role).

**Normal patients (y = 0) do not receive a cluster assignment in the DIANA application** - only the binary screening label ("Normal") is displayed. Cluster membership is shown only for at-risk patients, where it provides actionable subtype information (e.g., "At-Risk - SIRD phenotype: prioritize insulin resistance management"). This architectural decision prevents the algorithmic assignment of a disease phenotype to a healthy individual.
...
**Ahlqvist Subtype Label Assignment:** Clusters were assigned Ahlqvist-inspired subtype labels using a deterministic centroid-based algorithm adapted for the absence of HOMA2-B and C-peptide in NHANES:

1. **SIRD (Severe Insulin-Resistant Diabetes):** Assigned to the cluster with the highest LAP score, where LAP (Lipid Accumulation Product) = (WC - 58) * TG. This is a validated insulin resistance proxy per Wang et al. (2024, BMC Endocrine Disorders), who demonstrated LAP's superiority over BMI for identifying insulin resistance in NHANES cohorts.

2. **SIDD (Severe Insulin-Deficient Diabetes - Rebranded as Atherogenic/Lipid-Driven):** Assigned to the cluster with highest LDL cholesterol among remaining clusters. True SIDD identification requires HOMA2-B or C-peptide (beta-cell function markers unavailable in NHANES). Following Tanabe et al. (2024), we use high LDL as a proxy for the atherogenic dyslipidemia phenotype that characterizes this subtype.

3. **MOD (Mild Obesity-Related Diabetes):** Assigned to the cluster with highest BMI among remaining clusters, using the **Asia-Pacific WHO cutoff (BMI >= 25 kg/m²)** per Philippine clinical guidelines. This localized adaptation is critical - standard WHO cutoffs (BMI >= 30) routinely fail to detect metabolic risk in Asian populations, who exhibit higher body fat percentage and cardiovascular risk at lower BMI thresholds. By calibrating the MOD identification to Asia-Pacific criteria, DIANA successfully adapts a model trained on US NHANES data for deployment in Filipino populations - a significant localization achievement for global health equity.

4. **MARD (Mild Age-Related Diabetes):** Assigned to the residual cluster, typically characterized by older age at diagnosis and milder metabolic dysfunction.

**Inverse Transformation:** Cluster centers were inverse-transformed from standardized space back to raw clinical units before label assignment, ensuring clinically meaningful centroid interpretation (e.g., "SIRD centroid: BMI=32.4, TG=210 mg/dL, HDL=38 mg/dL").
...
**Implementation Reference:** Ian_ML/training/clustering.py:71-150, 474-480 and train_binary_v2_no_bp.py:685-750
...
| cluster | string | Ahlqvist subtype (SIRD/SIDD/MOD/MARD); null for Normal |
...
To evaluate whether the K-Means clustering merely rediscovered the pre-diabetic/diabetic split, we analyzed class distribution within each cluster. **Table 4.3** shows each cluster contains both labels in varying proportions, confirming clustering captures metabolic subtypes **orthogonal to glycemic severity**.

**Table 4.3 - Label Distribution Within At-Risk Clusters**

| Cluster | n | % of At-Risk | Mean Age | % Pre-diabetic | % Diabetic | Clinical Implication |
|---------|---|--------------|----------|----------------|------------|----------------------|
| SIRD | 404 | 29.4% | 58.3 | 72% | 28% | Insulin resistance (metformin first-line) |
| SIDD | 97 | 7.1% | 56.1 | 45% | 55% | Cardiovascular risk (statin therapy) |
| MOD | 370 | 26.9% | 57.8 | 68% | 32% | Weight management intervention |
| MARD | 505 | 36.7% | 61.2 | 81% | 19% | Conservative management |

**Interpretation:** SIDD shows highest diabetic proportion (55%), consistent with atherogenic phenotype. MARD has highest pre-diabetic proportion (81%) and oldest age (61.2y). This heterogeneity confirms clustering provides **actionable subtype stratification beyond binary risk**.
