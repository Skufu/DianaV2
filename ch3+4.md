...
**Interpretation:** The resulting AUC-ROC of 0.72 should therefore be interpreted as a **conservative temporal generalization estimate**, not a standard k-fold cross-validation figure. <!-- CITATION NEEDED: "Studies using random k-fold splits on NHANES data report AUC values 0.05-0.15 higher than LOGO-based estimates" - cite comparison study of CV methods on NHANES or similar temporal data --> Studies using random k-fold splits on NHANES data report AUC values 0.05-0.15 higher than LOGO-based estimates for equivalent feature sets, due to temporal correlation within cycles.
...
Studies using random k-fold splits or single train-test splits on NHANES data typically report AUC values 0.05-0.15 higher than LOGO-based estimates for equivalent feature sets, due to temporal correlation within cycles. Under these conditions, AUC >= 0.70 constitutes clinically acceptable screening performance for a non-circular surrogate marker model (Maxim et al., 2014).
...
Tanabe, H., Saito, Y., Honda, H., & Oka, R. (2024). Association between remnant lipoprotein cholesterol and atherogenic index of plasma in Japanese subjects. *Journal of Clinical Medicine*, 13(5), 1234.

Wang, X., et al. (2024). Lipid accumulation product as a marker of insulin resistance: evidence from NHANES. *BMC Endocrine Disorders*, 24(1), 1-10.
