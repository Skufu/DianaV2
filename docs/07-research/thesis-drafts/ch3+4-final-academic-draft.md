...
For planned user acceptance testing, the target recruitment locale consists of selected online communities of Filipino women discussing perimenopause and menopause-related health concerns and community-based recruitment in Nagcarlan, Laguna. The source protocol identifies the "Usapang Perimenopause at Menopause" Facebook interest group as one intended online recruitment setting. Nagcarlan, Laguna is included as a local community recruitment setting for usability evaluation only. Formal recruitment will proceed only after permission from relevant gatekeepers where applicable, informed consent, privacy safeguards, and the final testing protocol are completed.
...
The second population consists of community end-users recruited for user acceptance testing from selected Facebook interest groups and from Nagcarlan, Laguna. These participants will be selected using non-probability purposive sampling based on predefined criteria, including self-identification as perimenopausal, menopausal, or postmenopausal Filipino women and willingness to participate in the evaluation. The target sample will consist of at least 30 participants in total across the recruitment sources, unless the final UAT protocol requires separate source-specific quotas. These participants will interact with the DIANA web application and answer a Likert-scale questionnaire evaluating usability, clarity, interface understandability, and perceived usefulness for screening-risk awareness. The UAT participants will not be used for model training or model validation.

The clinical expert-review population consists of licensed medical professionals who can review risk-output plausibility, feature usefulness, SHAP explanation clarity, and clinical workflow fit. Formal community user acceptance testing has not yet been completed; however, an initial hands-on doctor expert review was completed and is reported as qualitative face-validity evidence rather than as external clinical validation.
