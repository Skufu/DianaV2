...
Phase 6 covered web application integration and visualization development, including frontend assessment workflows, backend API orchestration, ML-service integration, explainability output handling, trend visualization, and report-generation support. Phase 7 covered system testing and technical validation across backend, ML, frontend, security, deployment-readiness, and accessibility-readiness components. Phase 8 covered the completed community user acceptance testing and completed doctor's evaluation and expert-review procedure, separating user-acceptance evidence and expert face-validity feedback from external clinical validation claims. The methodological phases describe how the study was conducted; the corresponding empirical findings and validation evidence are reported in Chapter 4.
...
flowchart TB
    ...
    P8["Phase 8<br/>User acceptance testing,<br/>doctor's evaluation,<br/>and expert review"]
...
| Phase 8: User acceptance testing, doctor's evaluation, and expert review | 3.14 | 4.10 |
...
For community user acceptance testing, the recruitment locale consisted of selected online communities of Filipino women discussing perimenopause and menopause-related health concerns and community-based recruitment in Nagcarlan, Laguna. The source protocol identified the "Usapang Perimenopause at Menopause" Facebook interest group as one intended online recruitment setting. Nagcarlan, Laguna was included as a local community recruitment setting for usability evaluation only. The community UAT was used to evaluate usability and acceptance of the DIANA prototype, not to train or validate the predictive model.

### 3.3 Phase 1: Population of the Study

The study population consisted of distinct groups that contributed to different phases of the research. The primary modeling population was the secondary-data cohort derived from NHANES. These records were selected using predefined eligibility criteria rather than direct recruitment of participants. The final analytic cohort contained 1,376 postmenopausal women who satisfied the study's demographic, reproductive-health, and fasting-laboratory data-availability criteria. The cohort was restricted to female respondents within the target menopausal age range, with postmenopausal status derived from reproductive-health questionnaire responses. Reproductive-health filtering used RHQ031, which identifies respondents who reported no menstrual period during the past 12 months (CDC/NCHS, 2024c).
...
The second population consisted of community end-users recruited for user acceptance testing from selected Facebook interest groups and from Nagcarlan, Laguna. These participants were selected using non-probability purposive sampling based on predefined criteria, including self-identification as perimenopausal, menopausal, or postmenopausal Filipino women and willingness to participate in the evaluation. The community UAT sample consisted of at least 30 participants in total across the recruitment sources. These participants interacted with the DIANA web application and answered a Likert-scale questionnaire evaluating usability, clarity, interface understandability, and perceived usefulness for screening-risk awareness. The UAT participants were not used for model training or model validation.

The clinical expert-review population consisted of licensed medical professionals who reviewed risk-output plausibility, feature usefulness, SHAP explanation clarity, and clinical workflow fit. The doctor expert review is reported as face-validity evidence rather than as external clinical validation.
...
Operationally, each outer LOGO iteration followed the same sequence. First, one NHANES survey release was held out as the temporal test fold. Second, the remaining releases formed the development partition used for preprocessing, model fitting, hyperparameter selection, and threshold selection. Third, imputation, scaling, and model-selection steps were fitted only within the development partition before being applied to the held-out release. Finally, the selected model and threshold were evaluated once on the held-out release. This procedure reduced optimistic bias by preventing the held-out fold from influencing preprocessing, tuning, threshold selection, or model choice.
...
Software quality evaluation followed ISO/IEC 25010-informed characteristics (International Organization for Standardization, 2023). Functional suitability was evaluated through API tests, model-serving tests, and frontend unit tests. Performance efficiency was evaluated through inference benchmarks and planned load-testing methodology. Security was evaluated through authentication, role-based access, rate-limiting, and request-handling tests. Maintainability was supported through modular architecture, structured database access, and separated frontend, backend, and ML services. Community usability was evaluated through user acceptance testing, while accessibility certification, long-term reliability testing, and production load testing require separate operational evaluation. The completed doctor review is reported separately as qualitative expert face-validity feedback rather than as formal clinical validation.
...
**Phase 8 Start: User Acceptance Testing, Doctor's Evaluation, and Expert Review.** Phase 8 begins after the prototype has been implemented and technically reviewed. This phase documents the completed community user acceptance testing and the completed qualitative doctor expert review while separating usability and face-validity evidence from external clinical validation.

### 3.14 Phase 8: User Acceptance Testing, Doctor's Evaluation, and Expert Review Methodology

The community user evaluation followed an ISO/IEC 25010-informed usability framework (International Organization for Standardization, 2023). The protocol evaluated appropriateness recognizability, learnability, operability, user error protection, interface aesthetics, accessibility, and user confidence. User participants interacted with the DIANA prototype, completed core tasks such as logging in, navigating the dashboard, submitting an assessment, and interpreting prediction results, and answered a Likert-scale questionnaire on usability, clarity, understandability, and perceived usefulness.
...
| Phase 8 | Community UAT and doctor expert-review evidence | Analyze completed community UAT and completed doctor expert review while separating usability and face-validity evidence from clinical validation | User-acceptance and expert face-validity findings |
...
Excluding normal-glycemic (healthy) respondents from the clustering cohort was a deliberate design decision motivated by both mathematical and clinical considerations. Mathematically, K-Means clustering partitions the feature space by minimizing global distance variance. Injected normal-glycemic records (which make up 46.7% of the cohort and exhibit highly uniform, low-risk metabolic markers) would dominate the clustering process, causing significant centroid drift. This density shift would pull all centroids toward the normal range, contaminating the MARD-like cluster (the mildest metabolic phenotype) and diluting the mathematical contrast of the pathological patterns. Clinically, the Ahlqvist et al. (2018) subgroups were explicitly designed to describe the heterogeneity of type 2 diabetes pathology (insulin resistance, insulin deficiency, obesity-induced stress, and senescent metabolic decline). Assigning a pathological subtype label (such as SIRD-like or SIDD-like) to a healthy individual with normal glycemic biomarkers lacks clinical face validity and would introduce clinical mismatch. Therefore, restricting clustering to the at-risk partition ensures that the model isolates and characterizes the specific metabolic patterns of disease progression.
