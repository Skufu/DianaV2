...
DIANA implements a three-layer leakage detection system that serves as a mandatory pre-training gate. The validation pipeline is executed as Step 3 of 5 in the ML training workflow and terminates with a non-zero exit code on any failure, making leakage prevention a computationally verified safeguard rather than a design intention.
...
These checks were executed as a mandatory pre-training gate, making the non-circularity claim computationally verified rather than asserted by design intent alone.
