...
|-------|-------|-------------|---------------|-------|
| 2021-2023 | 3-year | `_L` | COVID-adapted | Most recent available; resumed August 2021 after pandemic suspension |
...
| Deployment | Vercel + Caddy-backed VPS + NeonDB | Static frontend hosting, reverse-proxy TLS termination, containerized backend/ML services, and managed PostgreSQL |
...
flowchart TB
    ...
    subgraph Proxy["Ingress Layer - VPS"]
        P[Caddy Reverse Proxy<br/>Let's Encrypt TLS]
    end

    subgraph Backend["Backend Layer - VPS Docker"]
        B[Go 1.25 + Gin<br/>JWT Auth + RBAC + Business Logic]
    end

    subgraph ML["ML Inference Service - VPS Docker"]
        C[Python 3.12 + Flask<br/>Logistic Regression + K-Means + SHAP]
    end

    subgraph Data["Data Layer - NeonDB + Optional Cache"]
...
The verified production deployment uses a **modular but manually triggered deployment strategy**. GitHub Actions CI validates the repository, while the VPS deployment is performed by a server-local script rather than by an automated GitHub Actions CD job. This distinction matters because the current production system does not deploy the frontend, backend, ML service, and database as one full-stack compose bundle.

- **Frontend**: deployed separately as a static React build on Vercel
- **Backend**: built and restarted as a Go Docker container on the VPS
- **ML Service**: built and restarted as a Python Flask/Gunicorn Docker container on the VPS
- **Database**: managed externally through NeonDB

The VPS deployment script pulls the selected revision, builds backend and ML images sequentially to avoid memory pressure on the small VPS, runs database migrations against NeonDB, and restarts Docker Compose services. Post-deployment checks verify that Caddy routes API requests to the backend and that the ML service remains private.
...
- **Public ingress:** Caddy is the only public reverse-proxy entry point for HTTP/HTTPS traffic; backend and ML services are not bound directly to public host ports
...
- **Secret handling:** Production secrets are loaded from a locked environment file and are not embedded as raw connection strings or JWT secrets in reverse-proxy or compose configuration
...
| SSL/TLS | Caddy with Let's Encrypt auto-renewal | Encrypted transport; prevents man-in-the-middle |
| Public Ingress Control | Only reverse proxy exposes host ports 80 and 443 | Reduces attack surface by preventing direct backend, ML, or database exposure |
...
| Secret Management | Locked environment file with no raw secrets in proxy or compose configuration | Reduces accidental credential disclosure |

**Implementation Reference:** `backend/internal/http/middleware/auth.go`, `backend/internal/http/middleware/rbac.go`, VPS reverse-proxy and compose configuration

---

#### 3.5.6 Deployment Architecture

The current DIANA production deployment uses a hybrid cloud/VPS architecture. The React frontend is deployed separately on Vercel, while the backend API and Python ML service run as Docker containers on a self-managed VPS behind Caddy. Caddy terminates TLS through Let's Encrypt and forwards API traffic to the backend container. The PostgreSQL database is provided by NeonDB as an external managed service, so no database server or database port is exposed from the VPS.

Because the frontend and database are managed externally, Docker Compose on the VPS is limited to the services required for API routing and ML inference. This keeps the production host smaller while preserving separation between presentation, inference, and persistence layers.
...
| Reverse Proxy | VPS-hosted Caddy | Let's Encrypt TLS termination; forwards public API traffic to backend |
| Backend API | VPS Docker container | Go/Gin API, internal container port only |
| ML Service | VPS Docker container | Python Flask/Gunicorn service reachable only over Docker network |
...
3. Server-local deployment script pulls the selected application revision
4. Backend and ML Docker images are built sequentially to fit the VPS memory budget
5. Goose migrations run against the external NeonDB target
6. Docker Compose restarts the backend, ML, and Caddy services
7. Health checks verify backend routing through Caddy and ML isolation from the public internet

**Scalability:**
- Frontend: Vercel Edge CDN (global, no scaling concerns)
- Backend: VPS containerized deployment; can be moved to a larger VPS or managed container host if load increases
- ML Service: Independently containerized; can be assigned more CPU/RAM or moved to a separate inference host
...
graph LR
    ...
    H[Server-Local Deploy Script] -.->|Pull, Build, Migrate, Restart| C
    H -.->|Build and Restart| D
```

**Implementation Reference:** `.github/workflows/ci.yml`, `docker-compose.yml`, `backend/cmd/server/main.go`, VPS Caddy configuration
...
- **Brier Score 0.2087**: Indicates acceptable overall probability accuracy. For context, a Brier score of 0 represents perfect prediction; values depend on prevalence and discrimination as well as calibration, so the metric should not be interpreted as calibration alone.
...
- **Hosmer-Lemeshow χ² = 24.75**: This statistic tests the null hypothesis that the model is well-calibrated. While the value suggests moderate fit (lower is better), Hosmer-Lemeshow is known to be sensitive to sample size and binning choices. The Brier score and ECE provide more reliable calibration assessment.
...
The weighted K-Means clustering (K=4) was assessed using three standard internal validation metrics computed on the at-risk subset (n=734). Missing clustering features are imputed before clustering rather than reducing the clustering analysis to complete cases only:
...
Functional testing was conducted using Go's built-in `testing` package, Python's `pytest` framework, and the frontend Vitest suite. The current codebase verification was re-run on 2026-05-14; results below distinguish passing suites from suites that currently require remediation.
...
**Table 4.4: Backend API Test Status as of 2026-05-14**

| Test Area | Current Status | Notes |
|-----------|----------------|-------|
| Full backend command: `go test ./...` | PASS | Completed successfully on 2026-05-14 |
...
The frontend Vitest suite is present and is executed by CI through `npm run test:coverage`. In the current working tree, the local frontend unit and contract tests pass.

| Test Area | Current Status | Notes |
|-----------|----------------|-------|
| `npm test` | PASS | 14 test files passed; 214 tests passed |
...
| `npm run test:coverage` | FAIL | Tests pass, but global coverage thresholds are not met: 33.56% lines/statements and 36.7% functions versus the configured 70% thresholds |

Remaining unit-test warnings are test-environment warnings from mocked animation/chart rendering and do not fail the suite. The coverage-threshold failure remains a release-readiness gap unless thresholds are recalibrated or additional frontend coverage is added.
...
A production VPS verification pass was performed to confirm that the deployed service exposure matched the intended security architecture. The check focused on externally reachable ports, reverse-proxy routing, ML service isolation, database exposure, and secret placement. Sensitive operational details such as server addresses, usernames, private paths, and connection strings are intentionally excluded from this manuscript.
...
The same verification identified stale Docker healthcheck probes for backend and ML containers. The backend service itself was reachable through the public reverse-proxy health endpoint, but the container-level healthcheck path used an outdated endpoint. The ML healthcheck likewise required alignment with the actual internal service port. This finding was treated as an operations-readiness correction rather than a failure of the public routing or isolation model.
...
| UAT-11 | Assessment Export | At least 1 assessment exists | 1. Navigate to Export 2. Select date range 3. Download CSV | CSV file downloaded with correct column headers and data | Protocol defined; not executed |
