...
**Reference Label Construction Detail**
...
### 3.6.1 Biomarker Classification Thresholds

The backend applies clinical cutoffs to categorize individual biomarker
values into risk tiers. These thresholds trigger validation warnings and
inform the clinical context displayed alongside predictions.

| Biomarker | Low Risk / Normal | Borderline / Elevated | High Risk |
|-----------|-------------------|----------------------|-----------|
| LDL (mg/dL) | < 100 | 100–129 | ≥ 130 |
| HDL (mg/dL) | ≥ 40 | — | < 40 |
| Triglycerides (mg/dL) | < 150 | 150–199 | ≥ 200 |
| Total Cholesterol (mg/dL) | < 200 | 200–239 | ≥ 240 |
| BMI (kg/m², Asia-Pacific) | < 23 | 23–24.9 | ≥ 25 |

Thresholds follow ATP III (NCEP, 2002) clinical guidelines, consistent
with the NHANES data source. The Asia-Pacific BMI cutoffs (WHO/IASO/IOTF,
2000) are used in place of Western thresholds (25/30) to better reflect
diabetes risk in Asian populations.

**HDL Threshold Justification (40 vs. 50 mg/dL):**

We selected the ATP III cutoff (HDL < 40 = low) rather than the American
Heart Association prevention target (HDL < 50 for women) as an engineering
constraint. Changing classification thresholds at inference creates a
train-inference mismatch — a form of distribution shift. NHANES data
uniformly uses ATP III cutoffs; applying different thresholds at inference
would produce predictions from a mismatched classifier, violating the
closed-world assumption. The 40 mg/dL cutoff preserves model-data
alignment as a matter of methodological hygiene.

This design decision is consistent with Ahlqvist et al. (2018) and Tanabe
et al. (2024), both of which use ATP III lipid thresholds in their
clustering methodologies. The clinical reference ranges are exposed in
the frontend as contextual guidance (HDL > 60 mg/dL optimal) with
borderline detection highlighting values approaching clinical thresholds.

**Implementation Reference:** backend/internal/config/config.go:93-132
(ClinicalThresholds struct), backend/internal/ml/validation.go:18-56
(ValidateBiomarkers), backend/internal/config/AGENTS.md (Clinical
Thresholds table).
...
#### 3.5.1 Technology Stack and Four-Layer Architecture
...
# Output Summary (timings vary by environment):
# ok    github.com/skufu/DianaV2/backend/internal/cache       ~8.9s
# ok    github.com/skufu/DianaV2/backend/internal/config      ~0.4s
# ok    github.com/skufu/DianaV2/backend/internal/http/handlers   ~1.5s
# ok    github.com/skufu/DianaV2/backend/internal/http/middleware ~0.8s
# ok    github.com/skufu/DianaV2/backend/internal/http/sse    ~0.8s
# ok    github.com/skufu/DianaV2/backend/internal/ml          ~1.4s
# ok    github.com/skufu/DianaV2/backend/internal/models      ~0.9s
# ok    github.com/skufu/DianaV2/backend/internal/pdf         ~1.0s
# ok    github.com/skufu/DianaV2/backend/internal/services    ~1.2s
# ok    github.com/skufu/DianaV2/backend/internal/store       ~1.2s
```

**Assessment Handler Clinical Guardrail Tests (Table-Driven)**

In addition to the package-level test counts above, a consolidated table-driven test (`TestAssessmentsHandler_Create_TableDriven`) validates five critical clinical safety invariants of the assessment creation endpoint. Each sub-test exercises a distinct guardrail path to ensure that the handler enforces population constraints, accepts missing-data imputation, surfaces biomarker validation warnings, and correctly invokes the ML predictor for valid inputs.
...
|---------|----------|---------------|---------------------|-----------------|
| TC-AGE-LO | Age below canonical range | age=44, valid biomarkers | 400 Bad Request | Canonical age policy error message returned; no assessment persisted; predictor not invoked |
| TC-AGE-HI | Age above canonical range | age=61, valid biomarkers | 400 Bad Request | Same age policy enforcement at upper boundary; no side effects |
...
| TC-SUCCESS | Successful end-to-end create | age=50, complete biomarker panel | 201 Created | ML predictor invoked; assessment persisted with correct age; no validation warnings for normal-range values |

**Test Design Rationale:**

- **Age boundary tests (TC-AGE-LO, TC-AGE-HI):** DIANA targets postmenopausal women aged 45–60. The handler enforces this constraint server-side (lines 435–438 in `assessments.go`) to prevent out-of-population predictions. The table-driven structure makes boundary values explicit and independently verifiable.

- **Missing waist circumference (TC-WC-IMP):** Waist circumference is a clustering feature but may not be available in self-screening contexts. When omitted, the Go handler passes 0 to the ML service, which applies BMI-concordant heuristic imputation (see Section 3.1.4.1). This test confirms the backend does not reject the request prematurely.

- **HbA1c out-of-range (TC-HBA1C-OOR):** The `ValidateBiomarkers()` function (ml/validation.go) flags extreme values as warnings in the `validation_status` field without blocking the prediction. This design reflects the clinical safety philosophy: extreme inputs receive flagged screening results rather than silent rejection, ensuring the clinician sees both the prediction and the warning.

- **Successful create (TC-SUCCESS):** Confirms the complete happy-path pipeline: JSON binding → age validation → biomarker validation → ML prediction → transactional persistence.

**Test Execution:**
```bash
cd backend && go test -v -run TestAssessmentsHandler_Create_TableDriven ./internal/http/handlers/

# Output:
# === RUN   TestAssessmentsHandler_Create_TableDriven
# === RUN   TestAssessmentsHandler_Create_TableDriven/TC-AGE-LO:_age_44_rejected_(below_canonical_minimum)
# === RUN   TestAssessmentsHandler_Create_TableDriven/TC-AGE-HI:_age_61_rejected_(above_canonical_maximum)
# === RUN   TestAssessmentsHandler_Create_TableDriven/TC-WC-IMP:_missing_waist_circumference_accepted_(imputation_path)
# === RUN   TestAssessmentsHandler_Create_TableDriven/TC-HBA1C-OOR:_HbA1c=20_triggers_diabetic-range_validation_warning
# === RUN   TestAssessmentsHandler_Create_TableDriven/TC-SUCCESS:_successful_create_invokes_ML_predictor_and_persists
# --- PASS: TestAssessmentsHandler_Create_TableDriven (0.00s)
# PASS
```
