...
**Stage 1 - Binary Screening (Gatekeeper):** All users are first evaluated by the logistic regression screening classifier, which outputs a binary risk label ("Normal" vs. "At-Risk"). This stage serves as the entry gate, ensuring that only users with sufficient metabolic risk proceed to subtype stratification. The classification uses the raw at-risk probability (from the classifier) compared against a pre-determined decision threshold (**0.465** for the current deployed model) to determine status.
...
A repository-level deployment-readiness review was performed to confirm that the checked-in deployment artifacts match the intended security architecture. A follow-up live external audit was performed on 2026-05-30 against the backend host `diana-v2.duckdns.org` and the configured frontend origin `https://diana-v2.vercel.app`. The review focused on reverse-proxy routing, backend health checks, service-port exposure, database exposure, runtime secret injection, live TLS behavior, production CORS behavior, and backend-mediated ML proxy behavior. Sensitive operational details such as usernames, private paths, tokens, and connection strings are intentionally excluded from this manuscript. The live audit also identified that current ML-service API-key enforcement is not active because the deployed ML service logs report `ML_API_KEY` as not configured.

| Verification Item | Expected Result | Observed Result | Status |
|---|---|---|---|
| Public ingress | Public traffic enters through reverse proxy | Live DNS resolved to `143.198.222.21`; ports 80 and 443 accepted connections; HTTP redirected to HTTPS; direct application ports 8080, 5000, 5001, and 5432 timed out externally | Live external exposure check passed; actual host firewall rule inventory still requires operator evidence |
...
| ML exposure control | ML service should not expose a direct container port publicly; browser-facing ML routes require explicit deployment controls | Direct public `/ml` and `/predict` paths returned 404; unauthenticated `/api/v1/ml/health` returned 401; authenticated `/api/v1/ml/insights/metrics` succeeded through backend proxy; live ML logs reported `ML_API_KEY` not configured | Network and backend-auth boundary passed; live ML-service API-key enforcement not active |
| Database exposure | PostgreSQL should not be exposed as a public application port in production configuration | Public port 5432 timed out externally; authenticated operations health reported database ping succeeded | Live exposure and health checks passed; runtime database TLS mode still requires operator-level verification |
...
The live review confirms effective public exposure limits, TLS availability, CORS allow-list behavior, and backend-mediated ML access for the checked endpoint. It does not replace production load testing, and it does not prove runtime database TLS mode because the active database connection string and database session settings are not exposed through public endpoints. A post-audit source change now makes production ML startup and Compose rendering fail when `ML_API_KEY` is absent, but this still does not support a live ML API-key-enforcement claim until `ML_API_KEY` is configured in the deployed backend and ML service and direct no-key/fake-key ML checks return 401.
