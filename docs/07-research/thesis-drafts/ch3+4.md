...
**Implementation Reference:** Ian_ML/training/train_binary_v2_no_bp.py:230-308
...
In the final cohort, **1.7%** (23/1,376) of records had at least one flagged outlier. The flag is available for sensitivity analyses, but row exclusion is not part of the active preprocessing output.

**Implementation Reference:** Ian_ML/training/data_processing.py:173-260
...
Redis 7 serves as the caching layer for frequently accessed data such as user assessment trends and analytics summaries. The **sub-millisecond latency** of Redis reduces database query load for repeated read operations. The backend combines TTL-based expiration with targeted invalidation after assessment writes for affected trend, summary, and cluster-distribution keys.
...
| Cache | Redis 7 | Sub-millisecond latency; cached analytics/trend data; TTL-based cache expiration and targeted invalidation |
...
flowchart TB
    ...
    subgraph Data["Data Layer - NeonDB + Redis"]
        ...
        E[(Redis 7<br/>Cache + TTL Invalidation)]
    end
...
The deployment job performs an SSH-based deployment using configured GitHub secrets. It stores previous image tags for rollback, updates the remote deployment environment, runs the deployment script, and performs backend, ML, and frontend health checks after startup.
...
4. SSH-based deployment updates the remote Docker Compose environment
5. Goose migrations → PostgreSQL deployment target
6. Health checks → backend `/api/v1/healthz`, ML `/health`, and frontend root endpoint
