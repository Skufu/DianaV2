...
### 3.1 Phase 1: Data Acquisition and Preprocessing Pipeline
...
### 3.2 Phase 2: Data Leakage Prevention Architecture
...
This three-layer architecture constitutes DIANA's methodological approach to leakage mitigation. To our knowledge, few T2DM ML screening tools include reproducible leakage validation procedures, which may address a gap in ML-based screening research.

**Implementation Reference:** Ian_ML/training/validate_no_leakage.py (entire file)

---

### 3.3 Phases 3-4: Machine Learning Methodology
...
#### 3.3.3 Phase 4: Clinical Threshold Optimization
...
- **False Negatives (missed cases):** Delayed confirmatory testing, delayed preventive counseling, and possible progression before review
...
- **Decision Curve & Net Benefit Theory:** Net benefit approaches mathematically drive the optimal decision threshold well below 0.5 when the benefit of a true positive (earlier confirmatory evaluation or prevention discussion) far outweighs the harm of a false positive (a low-cost FPG/HbA1c test) (Vickers & Elkin, 2006; Vickers et al., 2019).
...
#### 3.3.5 Phase 5: Metabolic Subtyping via Weighted K-Means

DIANA implements a **two-stage hierarchical architecture** that mirrors screening triage workflows:

**Stage 1 - Binary Screening (Gatekeeper):** All users are first evaluated by the logistic regression screening classifier, which outputs a base at-risk probability. The serving layer may then apply documented guardrails, including the MetS probability adjustment described in Section 3.5.2.1. The final runtime classification uses the served at-risk probability compared against the pre-determined decision threshold (**0.465** for the current deployed model). This stage serves as the entry gate, ensuring that only users classified with sufficient metabolic risk proceed to subtype stratification.

**Stage 2 - Weighted K-Means Subtyping (Stratifier):** Weighted K-Means clustering (K=4) is applied **exclusively to at-risk users** (those classified as "At-Risk", predicted_status = "At-Risk"), not the full cohort. This design is methodologically defensible because clustering is used to explore metabolic heterogeneity within the at-risk population, while the binary classifier performs the separate screening task. The serving code enforces this gating at runtime: subtype clustering logic only executes when predicted_status equals "At-Risk". K-Means was used as the centroid-based clustering foundation (MacQueen, 1967).

**Literature-Informed Heuristic Feature Weights:** The clustering weights function as feature scaling multipliers before Euclidean distance computation, using domain knowledge to shape geometric separation. Weights were derived through literature synthesis of metabolic biomarker importance in T2DM clustering and insulin resistance research. They are not medically validated clinical weights, causal effect sizes, or treatment priorities. Higher weights amplify a feature's influence on cluster separation. For a standardized user profile vector $\mathbf{z}_i$ and centroid $\boldsymbol{\mu}_k$, the weighted distance used for assignment is:

$$
d_w(\mathbf{z}_i,\boldsymbol{\mu}_k)=\sqrt{\sum_{j=1}^{p}w_j(z_{ij}-\mu_{kj})^2}
$$

where $w_j$ is the feature-specific weight, $z_{ij}$ is the standardized profile value for feature $j$, and $\mu_{kj}$ is the corresponding standardized centroid value for cluster $k$. The following weights are applied to the standardized features, reflecting an interpretable, literature-informed weighting scheme rather than empirically optimized causal importance:
...
**Normal predictions receive neutral sentinel subtype semantics** - specifically, the ML service returns `risk_cluster="N/A"`, `metabolic_subtype="N/A"`, `metabolic_subtype_full="N/A"` with empty `cluster_description` and `treatment_focus` fields. The backend canonicalization layer normalizes these neutral sentinels to blank cluster values at persistence, ensuring Normal assessments do not carry subtype cluster profiles in the database. Cluster membership is shown only for at-risk users, where it provides hypothesis-generating subtype context (e.g., "At-Risk - SIRD-like pattern: consider insulin-resistance-focused counseling or follow-up discussion"). This architectural decision prevents the algorithmic assignment of a disease phenotype to a healthy individual.

**Why At-Risk Only?** Fitting cluster centroids on the full cohort would distort subtype profiles by including Normal cases that, by definition, do not belong to any diabetic subtype. The scaler (StandardScaler) and imputer (SimpleImputer, median strategy) were fitted on the at-risk subset only, ensuring that standardization reflects the metabolic distribution of the target population. The runtime gating enforces this design invariant: KMeans prediction only runs for "At-Risk" predictions, and neutral sentinels are returned for "Normal" predictions.
...
### 3.4 Phase 6: Model Explainability and Clinical Decision Support
...
### 3.5 Phase 6: System Architecture and Implementation
...
If a user profile meets criteria for MetS based on waist, BMI, triglyceride, and HDL criteria aligned with commonly used metabolic-syndrome definitions (Waist ≥ 80cm, BMI ≥ 25, TG ≥ 150, HDL < 50), the serving layer (`predict.py`) adjusts the raw logistic regression probability using a pre-specified risk floor (International Diabetes Federation, 2006; Alberti et al., 2009):
- Meeting 2 criteria applies a flat `+0.15` probability boost.
- Meeting 3+ criteria enforces a minimum probability of `0.65`.

Operationally, the guardrail is applied after the Logistic Regression model produces its base probability. Let $\hat{p}_{model}$ represent the classifier probability and let $c$ represent the number of MetS criteria satisfied by the user profile. The served probability is:
...
This hybrid architecture (linear ML bounded by rule-based heuristics) is intended to reduce implausibly low risk estimates for users with multiple metabolic syndrome markers. Because the override is rule-based, it should be reported as an engineered safety heuristic and evaluated separately through ablation, calibration, and prospective review rather than treated as an independently validated clinical rule. The rule does not retrain the classifier, change the reference labels, alter model coefficients, or contribute to the reported cross-validation metrics.

**Evaluation Boundary:** The Chapter 4 AUC, sensitivity, specificity, calibration, and model-comparison metrics are computed from the cross-validated Logistic Regression classifier and its threshold policy before applying serving-layer Metabolic Syndrome probability boosts. Runtime classification uses the served probability, so the MetS override can change individual deployment responses. It is therefore documented as an inference guardrail, not as part of the reported cross-validation performance estimate.
...
### 3.6 Phases 7-8: Development and Evaluation
...
- **Service exposure control:** The production Compose overlay removes direct external ML container ports. The default thesis workflow routes prediction through the backend; the Nginx overlay also contains an optional `/ml/` reverse-proxy location, so any deployment that enables browser-facing ML traffic must protect it with configured ML API-key and CORS controls.
...
| SSL/TLS | Caddy or Nginx reverse-proxy TLS configuration, depending on deployment path | Supports encrypted transport when valid certificates are configured |
| Public Ingress Control | Reverse proxy handles public HTTP/HTTPS ingress | Reduces attack surface by avoiding direct service-port exposure |
| ML Service Exposure Control | Backend-mediated thesis workflow; no external ML container port in the production overlay; optional `/ml/` reverse-proxy path requires deployment controls | Limits direct ML service exposure without claiming that every topology has no public ML HTTP route |
...
| ML Service | Docker container or internal service | Python Flask/Gunicorn service reachable by backend; optional reverse-proxy `/ml/` route exists in the Nginx overlay |
...
graph LR
    ...
    P -.->|Optional /ml proxy in Nginx overlay| D
...
The user evaluation methodology for DIANA follows a structured protocol combining the System Usability Scale (SUS), task-completion metrics, and clinical face-validity assessment by domain experts (Brooke, 1996; Bangor et al., 2008). Planned community UAT participants will be recruited through selected Facebook interest groups and community-based recruitment in Nagcarlan, Laguna using non-probability purposive sampling. These participants are intended to evaluate usability and screening-risk communication only; they are not used for model training or model validation. The detailed methodology - including participant recruitment, evaluation instruments, data collection protocol, and analysis framework - is presented in Section 4.10 (User Acceptance Testing and Expert Feedback). Community UAT results remain pending; the completed doctor review is reported qualitatively because no formal expert Likert scoring instrument was completed.
...
The Python ML service test suite comprises 275 tests covering clustering algorithms, data leakage prevention, feature parity, prediction endpoints, server functionality, API key authentication, production API-key configuration failure behavior, drift detection, and training utilities. Using the project virtual environment, the current ML suite passes.
...
| Frontend coverage thresholds | PASS | Source coverage gates met: 71.26% lines/statements, 60.55% branches, and 44.24% functions |
...
**Layer 3 - Screening Guardrails and Next Steps**:
- **Biomarker Snapshot**: LDL, triglycerides, HDL, BMI, and age are displayed with simple status labels
- **Screening Guardrail**: The modal states that the output is AI-assisted screening support, not diagnosis
- **Follow-up Next Steps**: Follow-up suggestions are selected from the risk and subtype context
...
**Stage 5 - PDF Health Report Export**

To support shared discussion between users and their primary care providers, the platform integrates a comprehensive PDF generation service (`pdf_export_service.go`). This module compiles a user's chronological assessment history and current risk distribution into a structured digital report. This feature bridges the gap between user-led screening and follow-up clinical review.
...
A repository-level deployment-readiness review was performed to confirm that the checked-in deployment artifacts match the intended security architecture. A follow-up live external and operator-level audit was performed on 2026-05-30 against the backend host `diana-v2.duckdns.org`, the configured frontend origin `https://diana-v2.vercel.app`, and the deployed VPS configuration. The review focused on reverse-proxy routing, backend health checks, service-port exposure, database exposure, runtime secret injection, live TLS behavior, production CORS behavior, backend-mediated ML proxy behavior, host firewall posture, database TLS behavior, and ML API-key enforcement. Sensitive operational details such as usernames, private paths, tokens, secret values, and connection strings are intentionally excluded from this manuscript.

| Verification Item | Expected Result | Observed Result | Status |
|---|---|---|---|
| Public ingress | Public traffic enters through reverse proxy | Live DNS resolved to `143.198.222.21`; ports 80 and 443 accepted connections; HTTP redirected to HTTPS; direct application ports 8080, 5000, 5001, and 5432 timed out externally; UFW was active with default incoming deny and SSH allowed only from the Tailscale address range | Live external exposure and host firewall checks passed |
| TLS certificate chain | HTTPS presents a valid certificate for the host | Live certificate subject/SAN was `diana-v2.duckdns.org`, issued by Let's Encrypt E8, valid 2026-05-09 to 2026-08-07, with verification OK | Live TLS check passed |
| Production CORS | Configured frontend origin is allowed and unrelated origins are rejected | Preflight from `https://diana-v2.vercel.app` returned 204 with allow-origin and credential headers; unrelated/lookalike origins returned 403 | Live CORS check passed |
| Backend routing | Backend health endpoint exists for routing checks | `/api/v1/healthz` returned `{"status":"ok"}` on the live deployment | Live endpoint check passed |
| ML exposure control | ML service should not expose a direct container port publicly; browser-facing ML routes require explicit deployment controls | Direct public `/ml` and `/predict` paths returned 404; unauthenticated `/api/v1/ml/health` returned 401; backend and ML containers had `ML_API_KEY` configured after remediation; direct internal no-key and fake-key requests to `/insights/metrics` returned 401; authenticated backend ML proxy requests returned 200 | Live proxy-boundary and ML API-key enforcement checks passed |
| Database exposure | PostgreSQL should not be exposed as a public application port in production configuration | Public port 5432 timed out externally; authenticated operations health reported database ping succeeded; backend DSN used `sslmode=require`; redacted `psql` connection info against the same Neon DSN reported TLSv1.3 with `TLS_AES_256_GCM_SHA384` | Live exposure, health, and database TLS checks passed |
...
The live review confirms effective public exposure limits, TLS availability, CORS allow-list behavior, host firewall posture, backend-mediated ML access, database TLS for the configured backend DSN, and ML API-key enforcement for direct internal ML requests. It does not replace production load testing, penetration testing, or a formal clinical security assessment.

---

**Load Testing Results**

**Note:** Performance benchmarks remain pending for production-style load testing with 50+ concurrent users. The measurement methodology is documented above; actual values should be collected during a future pilot or deployment-readiness evaluation before production-scale performance claims are made.
...
**User Cohort (minimum n=30 total across recruitment sources)**:
- **Source**: Members of selected Facebook interest groups, including "Usapang Perimenopause at Menopause" where permission is granted, and community-based participants from Nagcarlan, Laguna
- **Inclusion Criteria**: Filipino women aged 45-65, currently experiencing perimenopause, menopause, or postmenopause symptoms, and able to interact with the DIANA application during the evaluation session
- **Recruitment Method**: Group administrator invitation for Facebook-based recruitment and community-based purposive recruitment in Nagcarlan, with consent forms distributed before participation
...
|-------|----------|-------------|--------|
| Participant Recruitment | Facebook group and Nagcarlan community invitations, consent forms | [TBD] | Not started |
| UAT Testing Sessions | Moderated sessions with at least 30 total community participants | [TBD] | Not started |
...
2. **Recruitment Bias**: Participants recruited from Facebook interest groups and Nagcarlan through purposive sampling may not represent the broader menopausal women population
...
**Bibliography boundary:** The clean submission bibliography is the reference list in `ch3+4-final-academic-draft.md`. This technical backup intentionally retains additional implementation and appendix-support references. Do not promote entries from this backup into the final submission unless they are cited in the clean draft and normalized to APA style.
...
Ahlqvist, E., Storm, P., Käräjämäki, A., Martinell, M., Dorkhan, M., Carlsson, A., ... & Groop, L. (2018). Novel subgroups of adult-onset diabetes and their association with outcomes: a data-driven cluster analysis of six variables. *The Lancet Diabetes & Endocrinology*, 6(5), 361-369. https://doi.org/10.1016/S2213-8587(18)30051-2
...
Tenenbaum, A., Fisman, E. Z., Motro, M., & Adler, Y. (2006). Atherogenic dyslipidemia in metabolic syndrome and type 2 diabetes: therapeutic options beyond statins. *Cardiovascular Diabetology*, 5, 20. https://doi.org/10.1186/1475-2840-5-20
...
Karapanagiotis, S., Benedetto, U., Mukherjee, S., Kirk, P. D. W., & Newcombe, P. J. (2023). Tailored Bayes: A risk modeling framework under unequal misclassification costs. *Biostatistics*, 24(1), 85-103. https://pmc.ncbi.nlm.nih.gov/articles/PMC9748575/
...
Sulistiowati, E., & Pradono, J. (2022). Development of a validated diabetes risk chart as a simple tool to predict the onset of diabetes in Bogor, Indonesia. *Journal of the ASEAN Federation of Endocrine Societies*, 37(1), 34-41. https://pmc.ncbi.nlm.nih.gov/articles/PMC9242663/

Ryu, K. S., Kang, H. Y. J., Lee, S. W., Park, H. W., You, N. Y., Kim, J. H., ... & Cha, H. S. (2020). Screening model for estimating undiagnosed diabetes among people with a family history of diabetes mellitus: A KNHANES-based study. *International Journal of Environmental Research and Public Health*, 17(23), 8903. https://pmc.ncbi.nlm.nih.gov/articles/PMC7730533/
...
Osei-Yeboah, J., Kengne, A. P., Owusu-Dabo, E., Schulze, M. B., Meeks, K. A. C., Klipstein-Grobusch, K., ... & Agyemang, C. (2023). Validation of prevalent diabetes risk scores based on non-invasively measured predictors in Ghanaian migrant and non-migrant populations — The RODAM study. *Primary Care Diabetes*, 18(1), 67-74. https://pmc.ncbi.nlm.nih.gov/articles/PMC10687695/

Poltavskiy, E., Kim, D. J., & Bang, H. (2016). Comparison of screening scores for diabetes and prediabetes. *Diabetes Research and Clinical Practice*, 118, 146-153. https://pmc.ncbi.nlm.nih.gov/articles/PMC4972666/

White, N., Parsons, R., Collins, G., & Barnett, A. (2023). Evidence of questionable research practices in clinical prediction models. *BMC Medical Research Methodology*, 23(1), 174. https://pmc.ncbi.nlm.nih.gov/articles/PMC10478406/

Xiong, X. F., Yang, Y., Lu, J., Zhao, Y. J., Zhao, S. P., Chen, Y. Y., ... & Wei, Y. S. (2020). Ensemble learning models based on noninvasive features for type 2 diabetes screening: Model development and validation. *JMIR Medical Informatics*, 8(6), e15431. https://doi.org/10.2196/15431

Vickers, A. J., van Calster, B., & Steyerberg, E. W. (2019). A simple, step-by-step guide to interpreting decision curve analysis. *Diagnostic and Prognostic Research*, 3, 18. https://pmc.ncbi.nlm.nih.gov/articles/PMC6777022/
...
Chen, Y., Zhou, Y., Liu, Y., & Zhang, T. (2025). Integrating convolutional neural networks with ensemble methods for diabetes prediction. *Frontiers in Medicine*, 12, 1657889. https://doi.org/10.3389/fmed.2025.1657889
...
Ohn, J. H., Kwak, S. H., Cho, Y. M., Lim, S., Jang, H. C., Park, K. S., & Cho, N. H. (2016). Performance of body mass index in predicting diabetes and hypertension in the Framingham Heart Study. *Diabetes & Metabolism*, 40(6), 497-504. https://pmc.ncbi.nlm.nih.gov/articles/PMC2881430/


Janghorbani, M., Amini, M., Willett, W. C., Gouya, M. M., Delavari, A., Alikhani, S., & Mahdavi, A. (2007). The metabolic syndrome among postmenopausal women in Gorgan. *International Journal of Endocrinology and Metabolism*, 10(1), 311-316. https://pmc.ncbi.nlm.nih.gov/articles/PMC3296160/

Alghamdi, M., Al-Mallah, M., Keteyian, S., Brawner, C., Ehrman, J., & Sakr, S. (2017). A combined strategy of feature selection and machine learning to identify predictors of prediabetes. *Journal of the American Medical Informatics Association*, 24(4), 775-781. https://pmc.ncbi.nlm.nih.gov/articles/PMC7647289/

Wei, H., Chen, Y., Liu, J., Yang, X., Li, W., & Li, Y. (2024). Association between high-density lipoprotein cholesterol and type 2 diabetes mellitus: A two-sample Mendelian randomization study. *Lipids in Health and Disease*, 23(1), 66. https://pmc.ncbi.nlm.nih.gov/articles/PMC10917910/
...
Ren, X., Chen, Z. A., Zheng, S., Han, T., Li, Y., & Liu, W. (2022). Triglyceride to high-density lipoprotein cholesterol (TG/HDL-C) ratio as a predictor of insulin resistance and β-cell function. *Frontiers in Endocrinology*, 13, 828581. https://doi.org/10.3389/fendo.2022.828581

Shen, L., Song, L., Li, H., Liu, B., Zheng, X., Zhang, L., ... & Yuan, J. (2023). Association of age at menopause with type 2 diabetes mellitus in postmenopausal women: A cross-sectional study. *Diabetes, Metabolic Syndrome and Obesity: Targets and Therapy*, 16, 229-237. https://pmc.ncbi.nlm.nih.gov/articles/PMC9871996/

Brand, J. S., van der Schouw, Y. T., Onland-Moret, N. C., Sharp, S. J., Ong, K. K., Khaw, K. T., ... & Wareham, N. J. (2013). Age at menopause, reproductive life span, and type 2 diabetes risk: Results from the EPIC-InterAct study. *Diabetes Care*, 36(4), 1012-1019. https://pmc.ncbi.nlm.nih.gov/articles/PMC3609516/

Kim, J. H., Lim, S., Park, K. S., Jang, H. C., & Choi, S. H. (2024). Four-years change of BMI and waist circumference are associated with metabolic syndrome in middle-aged women. *Obesity Research & Clinical Practice*, 18(2), 149-155. https://pmc.ncbi.nlm.nih.gov/articles/PMC11068757/

Yahaya, T. O., Salisu, T. F., Abdulrahman, Y. B., & Umar, A. K. (2024). Evaluating feature selection methods for accurate diagnosis of diabetes. *Informatics in Medicine Unlocked*, 45, 101431. https://pmc.ncbi.nlm.nih.gov/articles/PMC11674021/

Ahmed, F., Al-Habori, M., Al-Zabedi, E., & Saif-Ali, R. (2021). Impact of triglycerides and waist circumference on insulin resistance and β-cell function in non-diabetic first-degree relatives of type 2 diabetes. *BMC Endocrine Disorders*, 21(1), 124. https://doi.org/10.1186/s12902-021-00788-5

Huang, J., et al. (2023). Association between serum LDL-C concentrations and risk of diabetes: A prospective cohort study. *Frontiers in Endocrinology*. https://pmc.ncbi.nlm.nih.gov/articles/PMC10590678/

Kodama, S., et al. (2012). Body mass index, waist circumference, and the risk of type 2 diabetes mellitus: A meta-analysis. *Annals of Internal Medicine*. https://pmc.ncbi.nlm.nih.gov/articles/PMC2905837/

Li, L., et al. (2019). K-means clustering of overweight and obese population using quantile-transformed metabolic data. *Diabetes, Metabolic Syndrome and Obesity: Targets and Therapy*. https://pmc.ncbi.nlm.nih.gov/articles/PMC6711566/

*Note: Additional K-Means feature-weight support should remain in this technical backup or a separately named appendix until each entry is normalized and tied to a specific clean-draft claim.*
