...
The guardrail mechanism represents a defensible threshold policy choice rather than post-hoc tuning on the held-out test fold. Without guardrail arbitration, the vulnerable 2021-2023 fold would have exhibited greater specificity collapse, degrading screening usefulness. The guardrail reduces the risk that sensitivity-biased threshold selection sacrifices specificity below pre-specified minimums when temporal conditions shift.
...
|---------|---|--------------|----------|----------------|------------|--------------------------------------|
| MARD-like | 232 | 31.6% | 55.4 | 65.9% | 34.1% | Lower-intensity metabolic follow-up context (residual pattern) |
| MOD-like | 226 | 30.8% | 54.3 | 55.8% | 44.2% | Weight and central-adiposity counseling context |
| SIDD-like | 199 | 27.1% | 54.9 | 72.4% | 27.6% | Lipid-focused cardiovascular risk discussion context |
| SIRD-like | 77 | 10.5% | 54.5 | 44.2% | 55.8% | Insulin-resistance-focused follow-up context |
...
**Interpretation:** The cluster distribution suggests metabolic heterogeneity within the at-risk population. **SIRD-like** exhibits the highest diabetic proportion (55.8%), consistent with a more metabolically adverse insulin-resistance-related pattern in this cohort. **MARD-like** remains the largest group (31.6%) and is best interpreted as a heuristic residual pattern rather than evidence of a milder longitudinal course. **SIDD-like** (atherogenic phenotype proxy) exhibits the highest pre-diabetic proportion (72.4%), which may indicate lipid-dominant risk among participants not yet classified as diabetic, but progression cannot be inferred from this cross-sectional dataset. Overall, clustering provides hypothesis-generating subtype context beyond binary risk, not definitive treatment stratification.
...
*Figure 4.1 Candidate: Main Dashboard Interface*
The dashboard screenshot should be inserted only after capture from the running application. The implemented evidence source is `frontend/src/components/user/Dashboard_user.jsx`, which renders the risk distribution cards and recent assessment list.
...
*Figure 4.2 Candidate: Assessment Form with Real-Time Validation*
The assessment-form screenshot should be inserted only after capture from the running application. The implemented evidence source is `frontend/src/components/user/AssessmentForm.jsx`, which contains field-level validation, BMI calculation, and clinical input grouping.
...
4. Backend persists the assessment and prediction metadata to PostgreSQL; SHAP values are requested separately through the explainability flow and are not stored as JSONB in the assessment row
...
Detailed SHAP state is handled separately by `SHAPExplanation.jsx` after requesting the explainability endpoint; it is not part of the persisted assessment row.

*Figure 4.3 Candidate: ML Result Modal with SHAP Explanation*
[PLACEHOLDER: insert verified screenshot captured from the running application.] The implemented evidence sources are `frontend/src/components/common/MLResultModal.jsx` and `frontend/src/components/common/SHAPExplanation.jsx`.

*Figure 4.4 Candidate: SHAP Waterfall Plot Detail*
[PLACEHOLDER: insert verified SHAP detail screenshot captured from a completed assessment with explanation data.] No synthetic feature-attribution values are reported in this thesis draft.
...
*Figure 4.5 Candidate: Personal Trends Visualization*
[PLACEHOLDER: insert verified trends screenshot captured from an account with sufficient historical assessments.] The implemented evidence source is `frontend/src/components/user/PersonalTrends.jsx`.
