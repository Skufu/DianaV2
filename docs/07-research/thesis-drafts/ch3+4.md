...
For candidate threshold grid $\mathcal{T}=\{0.10,0.11,\ldots,0.89\}$, predicted labels are assigned as $\hat{y}_i(t)=1$ when $\hat{p}_i\ge t$ and $\hat{y}_i(t)=0$ otherwise. The threshold-selection metrics are:
...
The fold-specific strategy is selected from $t_J$, $t_S$, and $t_G$ using:
