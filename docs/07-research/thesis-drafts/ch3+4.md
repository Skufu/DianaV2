...
1. **Youden's J Index:** Maximizes Sensitivity + Specificity - 1, providing balanced discrimination. This is a standard statistical criterion for ROC cut-point selection.
...
**Final Threshold Selection:** The winning strategy per fold was selected by the composite clinical score defined above.
...
where w_j is the feature-specific weight, z_j is the standardized patient value for feature j, and c_kj is the corresponding standardized centroid value for cluster k. The following weights are applied to the standardized features, reflecting an interpretable, literature-informed weighting scheme rather than empirically optimized causal importance:
...
**Evaluation Boundary:** The Chapter 4 AUC, sensitivity, specificity, calibration, and model-comparison metrics are computed from the cross-validated Logistic Regression classifier and its threshold policy before applying serving-layer MetS probability boosts. The MetS override can change individual deployment responses and is therefore documented as an inference guardrail, not as part of the reported cross-validation performance estimate.
