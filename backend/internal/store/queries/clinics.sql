...
WITH clinic_stats AS (
    SELECT
        uc.clinic_id,
        COUNT(DISTINCT uc.user_id) AS total_patients,
        COUNT(a.id) AS total_assessments,
        AVG(a.risk_score) AS avg_risk_score,
        COUNT(CASE WHEN a.risk_score >= 70 THEN 1 END) AS high_risk_count,
        COUNT(CASE WHEN a.created_at >= DATE_TRUNC('month', NOW()) THEN 1 END) AS assessments_this_month
    FROM user_clinics uc
    LEFT JOIN assessments a ON uc.user_id = a.user_id
    WHERE uc.clinic_id = $1
    GROUP BY uc.clinic_id
)
SELECT
    COALESCE(cs.total_patients, 0)::int AS total_patients,
    COALESCE(cs.total_assessments, 0)::int AS total_assessments,
    COALESCE(cs.avg_risk_score, 0)::float8 AS avg_risk_score,
    COALESCE(cs.high_risk_count, 0)::int AS high_risk_count,
    COALESCE(cs.assessments_this_month, 0)::int AS assessments_this_month
