...
const biomarkerReference = [
  {
    ...
    diabetic: '≥ 25.0',
    description:
      'Ratio of weight to height. Uses Philippine (Asia-Pacific WHO) cutoffs. Higher BMI increases diabetes risk, especially with central obesity.',
  },
  {
    ...
    diabetic: '≥ 88',
    description:
      'Indicator of central obesity. Important component of metabolic syndrome assessment.',
  },
  {
    name: 'Triglycerides',
    unit: 'mg/dL',
    ...
    diabetic: '≥ 200',
    description:
      'Type of fat in the blood. Elevated levels heavily correlate with insulin resistance.',
  },
  {
    ...
    diabetic: '< 40',
    description:
      '"Good" cholesterol that helps remove other forms of cholesterol. Lower values indicate higher risk.',
  },
  {
    name: 'TG/HDL Ratio',
    unit: 'ratio',
    ...
    diabetic: '≥ 3.0',
    description:
      'Validated surrogate marker for insulin resistance. High ratios suggest atherogenic dyslipidemia.',
  },
];
...
const faqData = [
  {
    ...
    answer: (
      <span>
        The clusters (SIDD, SIRD, MOD, MARD) help your healthcare provider understand which type of
        diabetes you may have. Each cluster has different characteristics and may respond better to
        certain treatments. For example, SIDD may benefit from early insulin, while MOD often
        responds well to lifestyle changes. Your provider will use this information alongside other
        factors to personalize your care.
      </span>
    ),
  },
  {
    ...
    answer: (
      <span>
        The prediction model is trained on large population datasets and uses machine learning
        algorithms validated in clinical research. However, individual results may vary, and this
        tool should be used as one input among many in clinical decision-making. Always discuss
  ...
  },
  {
    ...
    answer: (
      <span>
        Yes, your cluster assignment can change as your health status changes. Weight loss,
        lifestyle modifications, medication changes, and disease progression can all affect which
        cluster best describes your condition. Regular reassessment is recommended.
      </span>
    ),
  },
  {
    ...
    answer: (
      <span>
        Our model predicts your underlying risk <strong>before</strong> clinical diabetes develops.
        FBS and HbA1c are diagnostic markers used to confirm diabetes. By focusing on metabolic
        biomarkers like BMI, Waist Circumference, and Triglycerides, the system can identify risk
    ...
    >
      ...
      >
        <div className="flex items-center gap-5">
          {Icon && (
            <div
              className={`p-4 rounded-2xl flex-shrink-0 transition-colors ${isOpen ? theme.iconBg : 'bg-gray-100'}`}
            >
              <Icon size={28} className={`${isOpen ? theme.icon : 'text-gray-500'}`} />
            </div>
          )}
          <h3
            className={`text-xl md:text-2xl font-bold transition-colors ${isOpen ? 'text-gray-900' : 'text-gray-700'}`}
          >
            {title}
          </h3>
        </div>
        ...
        {isOpen && (
          ...
          >
            <div
              ...
              className={`px-6 md:px-12 pb-8 pt-6 border-t ${theme.border} bg-white rounded-b-3xl`}
            >
              {children}
            </div>
          </motion.div>
    ...
    <div className="space-y-8 animate-fade-in pb-8">
      ...
      <div className="bg-gradient-to-r from-diana-forest to-diana-forest-light p-8 rounded-3xl text-white">
        ...
        <p className="text-blue-100 text-lg leading-loose">
          DIANA uses machine learning to analyze your biomarkers and assign you to one of four
          diabetes clusters. Each cluster represents a distinct subtype of Type 2 Diabetes with
          unique characteristics and treatment considerations. This personalized approach helps
        ...
        >
          {Object.entries(clusterEducation).map(([key, cluster]) => (
            <motion.div
              ...
              className={`glass-card bg-white rounded-3xl border-2 transition-all ${
                activeCluster === key
                  ? 'border-diana-forest shadow-xl'
                  : 'border-diana-sand hover:border-diana-forest/50 cursor-pointer'
              }`}
              onClick={() => {
            ...
            >
              <div className="p-6 md:p-8">
                <div
                  className={`flex flex-wrap items-start justify-between gap-4 mb-4 ${activeCluster === key && 'cursor-pointer'}`}
                  onClick={e => {
                    if (activeCluster === key) {
                      e.stopPropagation();
                      setActiveCluster(null);
                ...
                >
                  <div className="flex items-center gap-5 min-w-0 flex-1">
                    ...
                    <div className="min-w-0">
                      <h4 className="font-bold text-2xl text-diana-text-primary break-words">
                        {key}
                      </h4>
                      <p className="text-base text-diana-text-secondary font-medium break-words">
                        {cluster.name}
                      </p>
                    </div>
                  </div>
                  <motion.span
                    animate={
                      isReduced ? { rotate: 0 } : { rotate: activeCluster === key ? 180 : 0 }
                    }
                    transition={isReduced ? { duration: 0 } : { duration: 0.2 }}
                    className="mt-2"
                  >
                ...
                <p className="text-diana-text-primary text-lg mb-2 leading-relaxed font-medium">
                  {cluster.shortDesc}
                </p>
                ...
                <AnimatePresence>
                  {activeCluster === key && (
                    <motion.div
                      initial={
                        isReduced ? { height: 'auto', opacity: 1 } : { height: 0, opacity: 0 }
                      }
                      animate={{ height: 'auto', opacity: 1 }}
                      exit={isReduced ? { height: 'auto', opacity: 1 } : { height: 0, opacity: 0 }}
                      transition={
                    ...
                    >
                      <div className="mt-6 pt-6 border-t border-diana-sand space-y-8">
                        <p className="text-diana-text-primary text-lg leading-loose">
                          {cluster.fullDesc}
                        </p>

                        <div>
                          <h5 className="text-base font-bold text-diana-text-primary uppercase mb-4 tracking-wider">
                            Risk Factors
                          </h5>
                          <ul className="space-y-3">
                            {cluster.riskFactors.map(factor => (
                              <li
                                key={`${key}-risk-${factor.substring(0, 20).replace(/\\s+/g, '-')}`}
                                className="text-lg text-diana-text-secondary flex items-start gap-4"
                              >
                                <span className="text-diana-forest mt-1.5 min-w-[12px] text-xl">
                                  •
                                </span>
                                <span>{factor}</span>
                              </li>
                        ...
                        <div>
                          ...
                          <ul className="space-y-3">
                            {cluster.recommendations.map(rec => (
                              <li
                                key={`${key}-rec-${rec.substring(0, 20).replace(/\\s+/g, '-')}`}
                                className="text-lg text-diana-text-secondary flex items-start gap-4"
                              >
                        ...
                        <div
                          className={`p-6 rounded-2xl ${cluster.bgColor} ${cluster.borderColor} border`}
                        >
                          ...
                          <ul className="space-y-3">
                            {cluster.clinicalImplications.map(imp => (
                              <li
                                key={`${key}-imp-${imp.substring(0, 20).replace(/\\s+/g, '-')}`}
                                className="text-lg text-diana-text-secondary flex items-start gap-4"
                              >
                                <span
                                  className="mt-1.5 min-w-[12px] text-xl"
                                  style={{ color: cluster.color }}
                                >
                                  •
                                </span>
      ...
      <ExpandableCard title="Biomarker Reference Guide" icon={Activity} defaultOpen={true}>
        <div className="space-y-4 md:hidden">
          {biomarkerReference.map(bio => (
            <div
              key={bio.name}
              className="rounded-2xl border border-diana-sand/80 bg-white/80 p-5 shadow-sm"
            >
              <div className="space-y-2">
                <div className="font-bold text-lg text-diana-text-primary">
                  {bio.name}
                  {bio.unit && (
                    <span className="text-sm font-medium text-diana-text-muted"> ({bio.unit})</span>
                  )}
                </div>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b-2 border-diana-sand text-lg">
                <th className="text-left py-5 text-diana-text-secondary font-bold uppercase tracking-wider">
                  Biomarker
                </th>
                <th className="text-center py-5 text-green-700 font-bold whitespace-nowrap">
                  Optimal / Normal
                </th>
                <th className="text-center py-5 text-amber-700 font-bold whitespace-nowrap">
                  Borderline / At-Risk
                </th>
                <th className="text-center py-5 text-red-700 font-bold whitespace-nowrap">
                  Elevated / High Risk
                </th>
              </tr>
            </thead>
                ...
                <tr key={bio.name} className="border-b border-diana-sand">
                  <td className="py-6 pr-6">
                    <div className="font-bold text-xl text-diana-text-primary mb-2 flex items-baseline gap-2">
                      {bio.name}
                      {bio.unit && (
                        <span className="text-sm font-medium text-diana-text-muted">
                          ({bio.unit})
                        </span>
                      )}
                    </div>
                    <div className="text-base text-diana-text-secondary leading-relaxed max-w-lg">
                      {bio.description}
                    </div>
                  </td>
                  <td className="text-center py-6 text-green-700 font-bold text-xl">
                    {bio.normal}
                  </td>
                  <td className="text-center py-6 text-amber-700 font-bold text-xl">
                    {bio.prediabetic}
                  </td>
                  <td className="text-center py-6 text-red-700 font-bold text-xl">
                    {bio.diabetic}
                  </td>
                </tr>
              ))}
      ...
      <ExpandableCard title="Understanding Your Risk Score" icon={Lightbulb}>
        <div className="space-y-6">
          <p className="text-diana-text-primary text-base leading-loose">
            Your risk score is calculated using machine learning algorithms trained on large
            population datasets. The score represents the probability of developing or having Type 2
            Diabetes based on your biomarker profile.
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="p-6 rounded-xl bg-green-50 border border-green-200 text-center">
              ...
              <div className="text-sm text-diana-text-muted mt-3">
                Maintain healthy lifestyle, annual screening
              </div>
            </div>
            <div className="p-6 rounded-xl bg-amber-50 border border-amber-200 text-center">
              ...
              <div className="text-sm text-diana-text-muted mt-3">
                Lifestyle modifications, more frequent monitoring
              </div>
            </div>
            <div className="p-6 rounded-xl bg-red-50 border border-red-200 text-center">
              ...
              <div className="text-sm text-diana-text-muted mt-3">
                Consult healthcare provider, consider treatment
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="text-base text-amber-700 leading-loose">
              This tool is for educational and informational purposes only. It is not intended to
              diagnose, treat, cure, or prevent any disease. The risk predictions and cluster
              assignments are based on statistical models and should not replace professional
              medical advice. Always consult with a qualified healthcare provider for diagnosis and
              treatment decisions.
            </p>
          </div>
        </div>
