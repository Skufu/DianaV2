...
const InsightsHeader = ({ loading, error }) => (
  <div className="bg-gradient-to-r from-diana-forest to-diana-forest-light rounded-3xl p-8 md:p-10 text-white shadow-xl relative overflow-hidden mb-8">
    <div className="relative z-10">
      <h4 className="text-blue-200 font-bold uppercase tracking-wider text-sm mb-2">
        Insights & Clustering
      </h4>
    ...
    {error && !loading && (
      <div className="absolute top-6 right-6">
        <span className="text-sm font-bold text-rose-200 bg-rose-500/20 px-3 py-1 rounded-lg border border-rose-400/30">
          {error}
        </span>
      </div>
    )}
