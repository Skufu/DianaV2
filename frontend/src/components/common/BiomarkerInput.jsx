    ...
    <motion.div variants={fadeIn} initial="hidden" animate="visible" className="mb-4">
      ...
      <div className="relative rounded-md shadow-sm">
        <motion.input
          ...
          className={`w-full px-4 py-2 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500 ${error ? 'border-red-500' : 'border-gray-300'
            }`}
      ...
      <AnimatePresence mode="wait">
        {referenceRanges && rangeText && (
          <motion.p
            ...
            exit={{ opacity: 0 }}
            transition={{ duration: 0.2 }}
            className={`mt-1 text-sm ${rangeText === 'Normal range'
              ? 'text-green-600'
