...
const generateMockUsers = (count = 10, page = 1) => {
  ...
  test('should reactivate user → marked active again', async ({ page }) => {
    ...
    let targetUserActivated = false;

    await page.route(`**/admin/users/${targetUserId}/activate`, async route => {
      const request = route.request();
      targetUserActivated = true;
      return route.fulfill({
        status: 200,
        headers: corsHeaders,
        contentType: 'application/json',
    ...
    await page.route('**/admin/users*', async route => {
      ...
      const users = generateMockUsers(pageSize, pageParam).map(user => {
        if (user.id === targetUserId) {
          return { ...user, is_active: targetUserActivated };
        }
        return user;
      });
    ...
    expect(targetUserActivated).toBe(true);

    await expect(page.locator('text=User activated')).toBeVisible({ timeout: 5000 });
    ...
    const refreshedStatusBadge = refreshedTargetRow.locator('td:nth-child(3) span');
    await expect(refreshedStatusBadge).toContainText('Active');
  });
});
