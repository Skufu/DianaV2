...
Furthermore, comparative studies highlight that Random Forest can improve predictive
performance on structured clinical data while regression-based models retain interpretability.
For this study, algorithms are limited to Logistic Regression and Random Forest to balance
clinical explainability with performance on the NHANES cohort.

In addition, a study by Abdulhadi and Al-Mousa (2021) applies multiple ML classification
methods on both the PIMA diabetes dataset and original questionnaire data. Results reveal
...
and external validation, which affect model generalizability and clinical trust. Complex models,
while powerful, often lack transparency, posing difficulties for healthcare practitioners who require
clear, evidence‑based reasoning in diagnosis and treatment recommendations. These limitations
indicate the need for models that are not only accurate but also user‑friendly, interpretable, and
...
uses non‑probability purposive sampling to select NHANES cycles, contextual participants, and
...
This study uses NHANES 2009–2023 cycles as the primary modeling dataset for postmenopausal
women aged 45 to 60 years. NHANES provides standardized laboratory and demographic fields
for metabolic biomarkers central to this study including fasting blood sugar, HbA1c, lipid panels,
and related indicators. The resulting cohort (approximately 1,376 records after filtering and
completeness checks) provides a consistent, reproducible dataset for model development while
Philippine hospital partnerships are still in progress.
...
The primary modeling dataset comprises NHANES records of postmenopausal
women aged 45 to 60 years, yielding approximately 1,376 complete records with documented
biomarker data (Fasting Blood Sugar, HbA1c, lipid parameters, age, BMI, menopausal status, and
family history). Records are filtered to include participants with core biomarker completeness and
valid measurement ranges suitable for machine learning analysis.
