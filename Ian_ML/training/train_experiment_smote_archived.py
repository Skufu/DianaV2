...
def engineer_features_reduced(df: pd.DataFrame) -> pd.DataFrame:
    ...
    - Added AIP (Atherogenic Index of Plasma) - validated insulin resistance surrogate
    - Added lipid ratios (LDL/HDL, Non-HDL) for atherogenic risk
    ...
    registry: dict[str, dict] = {
        ...
        "Random Forest": {
            ...
            "param_distributions": {
                ...
                "model__min_samples_leaf": randint(2, 50),
...
def run_nested_logo_evaluation(
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(outer_cv.split(X, y, groups), start=1):
        ...
        for model_name, cfg in model_registry.items():
            try:
                # Use SMOTE+Tomek for robust handling of outliers and class imbalance
                pipeline = Pipeline(
                    [
                ...
                print("   [WARN] imblearn not found or import failed, falling back to simple pipeline (no SMOTE)")
            ...
            search = RandomizedSearchCV(
                ...
                n_iter=20,  # Try 20 random combinations per fold
...
def train_full_model_for_serving(
    ...
    # Pipeline construction for full model serving
    try:
        pipeline = Pipeline(
            [
                ("imputer", SimpleImputer(strategy="median")),
    ...
    search = RandomizedSearchCV(
        estimator=pipeline,
        param_distributions=cfg["param_distributions"],
        n_iter=50,  # Try 50 combinations for final model
        scoring="roc_auc_ovr_weighted",
